...
int	Renderer::getVideoModeCount() {
	...
	static const D3DFORMAT kModeFormats[] = {
		D3DFMT_A8R8G8B8,
		D3DFMT_X8R8G8B8,
		D3DFMT_R8G8B8,
		D3DFMT_R5G6B5
	...
	for (int pass = 0 ; pass < 2 ; ++pass) {
		...
		for (int formatIndex = 0 ; formatIndex < (int)(sizeof(kModeFormats) / sizeof(kModeFormats[0])) ; ++formatIndex) {
			D3DFORMAT format = kModeFormats[formatIndex];
...
void	Renderer::init(const VideoMode &mode) {
	...
	D3DFORMAT searchFormat = D3DFMT_UNKNOWN;
	if (mode.bitsPerPixel == 16) {
		searchFormat = D3DFMT_R5G6B5;
	} else if (mode.bitsPerPixel == 24) {
		searchFormat = D3DFMT_X8R8G8B8;
	} else if (mode.bitsPerPixel == 32) {
		searchFormat = D3DFMT_A8R8G8B8;
	} else {
		assert(false);
	}
	...
	for (UINT modeIndex = 0 ; ; ++modeIndex) {
		...
		result = pD3D->EnumAdapterModes(D3DADAPTER_DEFAULT, searchFormat, modeIndex, &d3dMode);
		...
		if (d3dMode.Width == mode.xRes && d3dMode.Height == mode.yRes) {
			foundMode = true;
			break;
		}
	}
