...
void K_HandleEvents(game_state_t *game_state, player_t *player) {
    ...
    switch (event.type) {
        case SDL_EVENT_KEY_DOWN:
            K_HandleRealtimeKeys(event.key.scancode, KEY_STATE_DOWN);
            game_state->state_show_map = keystates.map_state;

            if (event.key.scancode == keymap.quit) {
                game_state->is_running = false;
            }

            if (event.key.scancode == keymap.debug_mode) {
        ...
        case SDL_EVENT_KEY_UP:
            K_HandleRealtimeKeys(event.key.scancode, KEY_STATE_UP);
            break;
...
void K_ProcessKeyStates(player_t *player, double delta_time) {
    if (keystates.forward) {
        player->position.x += cos(player->dir_angle) * MOV_SPEED * delta_time;
        player->position.y += sin(player->dir_angle) * MOV_SPEED * delta_time;
